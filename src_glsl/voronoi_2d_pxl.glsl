...
float rand_i2o1(vec2 seed) {
	return (sin( dot(seed.xy, vec2(12.9898, 78.233)) * 43758.5453 ) + 1.0f) / 2.0f;
}
vec2 rand_i1o2(float seed) {
	return vec2( rand_i2o1(vec2(seed * 2.0f)), rand_i2o1(vec2(seed / 2.0f)) );
}

layout(location=0)uniform float cst_time;

void main()
{
	vec4 pxl_clr = vec4(1.0f);
	vec2 pxl_crd = psi.txr_crd.xy + cst_time;
