...
namespace NWG
{
	shader::shader(cstring name, shader_types shd_type) :
		a_data_res(name),
...
namespace NWG
{
	vtx_shader::vtx_shader(gfx_engine& graphics, cstring name) :
		shader(name, SHD_VERTEX), t_gfx_res(graphics),
		m_layout(graphics) { }
		...
		while (std::getline(stm_code, str_buf, ' ')) {
			if ((ncurr = str_buf.find("layout")) != -1) {
				...
				if ((ncurr = str_buf.find("in ")) != -1) {
					...
					m_gfx->new_res<shd_buf>(m_bufs.back());
...
namespace NWG
{
	pxl_shader::pxl_shader(gfx_engine& graphics, cstring name) :
		shader(name, SHD_PIXEL), t_gfx_res(graphics)
	{
	}
...
namespace NWG
{
	gmt_shader::gmt_shader(gfx_engine& graphics, cstring name) :
		shader(name, SHD_GEOMETRY), t_gfx_res(graphics)
	{
	}
...
namespace NWG
{
	Shader::Shader(gfx_engine& graphics, cstring name, shader_types sType) :
		t_ent(), a_gfx_res(graphics), ACodeRes(name),
