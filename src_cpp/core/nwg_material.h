...
{
	/// gfx_material class
	class NWG_API gfx_material : public t_gfx_res<gfx_material>, public a_data_res
	{
	public:
		using shd = mem_ref<shader>;
		using shaders = darray<shd>;
		using info = gfx_material_info;
	public:
		gfx_material(gfx_engine& graphics, cstring name);
		virtual ~gfx_material();
		// --getters
		inline const info& get_info() const	{ return m_info; }
		inline shd& get_shader(shader_types shd_type);
	...
	inline gfx_material::shd& gfx_material::get_shader(shader_types shd_type) {
