...
namespace NWG
{
	...
	bool gfx_material::remake(const gfx_material_info& info)
	{
		...
		if (info.source_vtx != "default") {
			...
			m_gfx->new_res<shader, vtx_shader>(m_shds.back(), "shd_vtx");
		...
		if (info.source_vtx != "default") {
			m_shds.push_back(mem_ref<shader>());
			m_gfx->new_res<shader, pxl_shader>(m_shds.back(), "shd_pxl");
		...
		if (info.source_gmt != "default" && false) {
			m_shds.push_back(mem_ref<shader>());
			m_gfx->new_res<shader, gmt_shader>(m_shds.back(), "shd_gmt");
...
namespace NWG {
	gfx_material::gfx_material(gfx_engine& graphics, const char* name) :
		t_ent(), a_gfx_res(graphics), a_data_res(name),
